...
public class RedstoneBugEntity extends HostileEntity {
    private RedstoneBugEntity.CallForHelpGoal callForHelpGoal;

    public RedstoneBugEntity(EntityType<? extends RedstoneBugEntity> entityType, World world) {
        super(entityType, world);
    }

    protected void initGoals() {
        this.callForHelpGoal = new RedstoneBugEntity.CallForHelpGoal(this);
    ...
    public static Builder createRedstoneBugAttributes() {
    ...
    public static boolean canSpawn(EntityType<RedstoneBugEntity> type, WorldAccess world, SpawnReason spawnReason, BlockPos pos, Random random) {
    ...
    static class WanderAndInfestGoal extends WanderAroundGoal {
        ...
        public WanderAndInfestGoal(RedstoneBugEntity redstonefish) {
            super(redstonefish, 1.0D, 10);
    ...
    static class CallForHelpGoal extends Goal {
        private final RedstoneBugEntity redstonefish;
        private int delay;

        public CallForHelpGoal(RedstoneBugEntity redstonefish) {
            this.redstonefish = redstonefish;
        }
        ...
        public void tick() {
            --this.delay;
            if (this.delay <= 0) {
                World world = this.redstonefish.world;
                Random random = this.redstonefish.getRandom();
                BlockPos blockPos = this.redstonefish.getBlockPos();
                        ...
                        for(int k = 0; k <= 10 && k >= -10; k = (k <= 0 ? 1 : 0) - k) {
                            ...
                            if (block instanceof InfestedBlock) {
                                if (world.getGameRules().getBoolean(GameRules.DO_MOB_GRIEFING)) {
                                    world.breakBlock(blockPos2, true, this.redstonefish);
                                } else {
