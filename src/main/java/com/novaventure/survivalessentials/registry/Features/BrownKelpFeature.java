...
public class BrownKelpFeature extends Feature<DefaultFeatureConfig> {
    public BrownKelpFeature(Codec<DefaultFeatureConfig> config) {
        super(config);
    }
    ...
    public boolean generate(StructureWorldAccess structureWorldAccess, ChunkGenerator chunkGenerator, Random random, BlockPos blockPos, DefaultFeatureConfig defaultFeatureConfig) {
        ...
        if (structureWorldAccess.getBlockState(blockPos2).isOf(Blocks.WATER)) {
            BlockState blockState = BROWN_KELP.getDefaultState();
            BlockState blockState2 = BROWN_KELP_PLANT.getDefaultState();
